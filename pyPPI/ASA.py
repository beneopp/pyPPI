...
class ASA(object):
    ...
    def mesh(self, chain):
        ...
        for atom, neighbors in ASA.nearAtomsOnChain(chain, self.pdb.atoms, self.pdb.ktree):
            ...
            intraArea = ASA.calcASAforAtom(atom, intra_neighbors)
            interArea = ASA.calcASAforAtom(atom, inter_optimized, callback=self.print_point)
    ...
    def calcASAforAtom(atom, neigbors, callback=None):
        ...
        points_in = len(spiral_points)
        if callback:
            callback(spiral_points[:])
        area = area_calc(r, points_in, total_points)
        return area
    ...
    def print_point(self, points):
        ...
        """
        while self.q_points < 9999:
            for x, y, z in points:
                self.q_points += 1
                point_to_print = 'HETATM {0:^4}  O   HOH I{0:^4}    {1:^7.3f} {2:^7.3f} {3:^7.3f}  1.00 0.0             O '.format(self.q_points, x, y, z)
